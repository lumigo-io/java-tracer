...
public class SpansContainer {
  ...
  public void start() {
    this.startFunctionSpan = this.baseSpan
        .toBuilder()
    ...
    try {
      rttDuration = reporter.reportSpans(prepareToSend(startFunctionSpan, false), MAX_REQUEST_SIZE);
    } catch (Throwable e) {
  ...
  private void end(Span endFunctionSpan) throws IOException {
    this.endFunctionSpan = endFunctionSpan
        .toBuilder()
