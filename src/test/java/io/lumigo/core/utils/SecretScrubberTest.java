...
class SecretScrubberTest {
    ...
    void testSecretScrubbingUtils_does_not_scrub_non_json_payloads() {
        assertEquals("123", new SecretScrubber(noScrubbingOverridesEnv).scrubStringifiedObject("123"));
    }

    @Test
    @DisplayName("scrubs a nested body value with default expressions")
    void testSecretScrubbingUtils_scrubs_json_payload_default() {
        String actual = new SecretScrubber(noScrubbingOverridesEnv)
    ...
    @Test
    @DisplayName("scrubs body using the default patterns when LUMIGO_SECRET_MASKING_REGEX is invalid")
    void testSecretScrubbingUtils_scrubs_json_payload_default_used_when_invalid_pattern_list_provided() {
        String actual = new SecretScrubber(envWithMaskingRegex("[THIS IS NOT A VALID JSON ARRAY]"))
                .scrubStringifiedObject(
                        "{\"pass\":\"word\",\"key\":\"value\",\"secret\":\"stuff\",\"credential\":\"admin:admin\",\"passphrase\":\"SesameOpen\",\"SessionToken\":\"XyZ012x=\",\"x-amz-security-token\":\"amzToken123\",\"Signature\":\"yours truly\",\"authorization\":\"Bearer 123\"}");
        String expected = "{\"authorization\":\"****\",\"credential\":\"****\",\"pass\":\"****\",\"SessionToken\":\"****\",\"Signature\":\"****\",\"passphrase\":\"****\",\"secret\":\"****\",\"x-amz-security-token\":\"****\",\"key\":\"****\"}";
    ...
    void testSecretScrubbingUtils_scrubs_json_payload_default_case_insensitive() {
        String actual = new SecretScrubber(noScrubbingOverridesEnv)
                .scrubStringifiedObject(
                        "{\"pAss\":\"word\",\"KeY\":\"value\",\"seCRet\":\"stuff\",\"CrEdEntial\":\"admin:admin\",\"paSSPhrase\":\"SesameOpen\",\"seSSIOntOkEn\":\"XyZ012x=\",\"X-AMZ-security-token\":\"amzToken123\",\"SIGnatUre\":\"yours truly\",\"AuTHOrization\":\"Bearer 123\"}");
        String expected = "{\"pAss\":\"****\",\"CrEdEntial\":\"****\",\"paSSPhrase\":\"****\",\"seSSIOntOkEn\":\"****\",\"SIGnatUre\":\"****\",\"seCRet\":\"****\",\"X-AMZ-security-token\":\"****\",\"KeY\":\"****\",\"AuTHOrization\":\"****\"}";
    ...
    void testSecretScrubbingUtils_scrubs_json_payload_top_level_key() {
        String actual = new SecretScrubber(envWithMaskingRegex("[\".*topsecret.*\"]"))
                .scrubStringifiedObject("{\"topsecret\":\"stuff\"}");
    ...
    void testSecretScrubbingUtils_scrubs_json_payload_nested_key() {
        String actual = new SecretScrubber(envWithMaskingRegex("[\".*topsecret.*\"]"))
                .scrubStringifiedObject("{\"some\": {\"topsecret\":\"stuff\"}, \"a\": 1}");
