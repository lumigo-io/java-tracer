...
class SecretScrubberTest {
  ...
  void testSecretScrubbingUtils_scrubs_json_payload_default() {
    String actual = SecretScrubber.getInstance().scrubBody(
  ...
  @SetEnvironmentVariable(key = "LUMIGO_SECRET_MASKING_REGEX", value = "[THIS IS NOT A VALID JSON ARRAY]")
  ...
  void testSecretScrubbingUtils_scrubs_json_payload_nested_key() {
    String actual = SecretScrubber.getInstance().scrubBody("{\"some\": {\"topsecret\":\"stuff\"}, \"a\": 1}");
